...
## Project Overview
Our group chose pet adoption as our topic, because we are animal lovers. The purpose of this project is to create a machine learning model that predicts adoption speed, and to see what features affect the model. We are working with data from a previously completed Kaggle challenge and with a limited 4-week timeline. Because of that, we set our target accuracy at 42.3+% as that would have achieved ‘bronze’ if we had been participating in the original Kaggle competition. The top 10 (non-cheating) finalists were all at 44.1% or higher, with the best model at 44.99%.
...
Data cleaning and pre-processing for model [link to code](https://github.com/trosie3/UofODataViz_Final_Project/tree/main/Cleaning%26Preprocessing "link to code folder")
...
- The number of photos of the pet(s) will a have an impact. If looking online, a person probably looks for pictures and then scrolls through them as part of the decision making process.
  - Most pets have photos, and the majority of them have between 1 and 5 photos.
...
- A common thought is that black pets are less adoptable, so 'color1' might have more of an impact.
  - For both cats and dogs, the largest number of datapoints are pets at the shelter (speed 4). When comparing single-colored animals, a pattern is hard to discern, except that there appear to be more black pets in this dataset. 
...
- Description will likely have an impact. Not everyone reads descriptions, but many might, based on the fact that our whole team would read them.
  - Most description lengths (word_count) are under 250 words, with the three faster speeds being closer to 200. There does appear to be some correlation that as word count goes up, the timeline of the pet adoption also increases.
...
- Model 4
  - Features used: all but type, videoamt, health, vaccinated, sterilized
  - Classification Report:
...
80/20 split, cleaned_train.csv used (back to first csv because models 1 and 2 are the best models so far)
...
- Model 10 target accuracy hit / **Final Model**
  ...
  - Confusion Matrix and Classification Report: 
      ...
      - Actual 1 count 631 vs Predicted 1 count 589 : slightly under on total number, did fairly well at getting speed 1 right
...
Type, health and videoamt. We can safely say that these are the least important features, as our best performing models in both the 75/25 split and 80/20 split did not use these features, and their accuracy scores improved.
...
- Wrong : Type and health, they were actually two of the least important features. Also, single color wasn't as important as we suspected it might be. Type was actually really surprising to all of us, as we really thought dog versus cat would matter more.
- Right : word_count aka description (this feature turned out to be more important than we originally suspected it might), age and photoamt were also top features. All of these turned out to be top features, regardless of data/csv used.
- Missed : Color2 was more important than we would have guessed (in top 5 features), which makes sense in hindsight as most animals probably are dual-colored over a single color. Fee was also less important than we would have thought, even though we didn't expect it to be a top feature. 
...
- Focusing on the 'health' of the pet not necessary. When writing a description there is no need to mention that the pet(s) is healthy.

If these changes were made perhaps the next attempt at making a model for PetFinder would produce a model that could actually be used in making adoption speed predictions.
...
Time 
 - Given our 4-week project deadline, what we could perform and analyze was limited. If we had more time we could have:
   ...
   - Adjusted the bins/categories we made, as perhaps some of them weren't as representative of the data as they could have been. Better categorization could have increased accuracy.
   ...
   - Lastly, we could have performed a full Natural Language Processing breakdown on the description column instead of only looking at the word length in each description. By being able to analyze the content and sentiment of a description, it may have increased the accuracy of the model, as well as provided more insight as to what the description should say over just our suggestion for between 40-200words needed.
 
Data
  - Limited amount of data points in Adoption Speed 0, which is also where even our best model struggled predicting. Perhaps if there had been a more evenly distributed amount of data points in each adoption speed, the model could have been trained better, thus producing a model better at predicting adoption speed.
  - Adoption speed already pre-categorized. Given that adoption speed is based on a time-line, had this not previously been categorized, we could have performed a LinearRegression Model which could have given us more insight as to where/what the model was struggling to predict, and perhaps could have been a more accurate model.
  - With photoamt being a key feature, perhaps if the images were provided, an analysis of the images could have been performed, to key into what images help make the pets more adoptable. This could lead to suggestions on what those 1-5 photos should be rather than just the number suggested.

With more time and/or better data, perhaps we could have made a better model, and/or come up with more ways PetFinder could improve individual listings. Plus, improved listings could lead to fewer pets still in the shelter after 100 days, which had the largest count of data points in this dataset.
